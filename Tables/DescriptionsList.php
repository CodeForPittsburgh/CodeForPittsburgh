...
while ($row = pg_fetch_row($result)) {
    //print $row[0] . ' ' . $row[1] . ' '.$row[2] . '</br>'; 
    PopulateIncidentTable($row,$MinDate,$MaxDate);
}
...
function PopulateIncidentTable($values,$MinDate,$MaxDate) {
    /* Populate table with results. */
    echo "<tr>";
    //foreach ($values as $value) { 
    $my_href="<td><a href='../maps/PPBXMLAddressMarkerCreate.php?section=$values[0]&mindate=$MinDate&maxdate=$MaxDate'>$values[0]</a></td>";
    echo $my_href;
//echo "<td><a href='../maps/PPBXMLAddressMarkerCreate.php?section=$values[0]&mindate=' . $MinDate . '&maxdate=' . $MaxDate>$values[0]</a></td>";
    echo "<td>$values[1]</td>";
    echo "<td>$values[2]</td>";
    //}
    echo "</tr>";
